  ...
  return (
    <>
      <section className="hero scroll-hero" key={page.key} style={{ '--scroll-progress': heroProgress }}>
        ...
        <div className="hero-bottom"><div><p>{page.copy}</p>{page.key === 'home' && <button className="hero-dashboard-button hero-dashboard-primary" onClick={() => onNavigate('/dashboard')}>Open AIWithStudy <b>↗</b></button>}</div><button className="circle-button" onClick={() => document.querySelector('.below')?.scrollIntoView({ behavior: 'smooth' })} aria-label="Scroll to explore">↓</button></div>
